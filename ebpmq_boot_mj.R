...
#' @export
ebpmq_boot_mj<-
  function(xs,barX,area.s,betaq,sigma2u,sigma2e,est.method=c("local","global"),method=c("LBP","Naive"),Ni,ni,B=50,tol=1e-06,maxit=100,k_b=100,k_sigma_u=100,k_sigma_e=100,numCores=2)
  {#start function
    ...
    fun.boot<-function(h){
      ...
      list(Yibar.boot=Yibar.boot,MQBP.LBP.boot=MQBP.LBP.boot)
    }#end fun.boot
    ...
    a_phi.ebp<-NULL
    a_phi.ebp<-log(sqrt(apply((MQBP.LBP.boot-Yibar.boot)^2,2,mean)))
    list(a_phi.ebp=a_phi.ebp)
    #end function
  }
